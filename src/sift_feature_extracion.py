...
flags.DEFINE_string('gray_scale', 'no', 'String: "yes" or "no", if source image in output should be in grayscale')
...
def main():
    ...
    if FLAGS.features_only not in ['yes', 'no'] or FLAGS.rich_keypoints not in ['yes', 'no']:
        raise ValueError('invalid arguments in --features_only or --rich_keypoints')
...
def sift_extraction(images, classes, dest_dir):
    ...
    for i in range(len(images)):
        ...
        if FLAGS.features_only == 'yes':
            ...
            if FLAGS.rich_keypoints == 'yes':
                if FLAGS.gray_scale == 'yes':
                    features = cv2.drawKeypoints(gray, kp, blank_image,
                                                 flags=cv2.DRAW_MATCHES_FLAGS_DRAW_RICH_KEYPOINTS)
                else:
                    features = cv2.drawKeypoints(image, kp, blank_image,
                                                 flags=cv2.DRAW_MATCHES_FLAGS_DRAW_RICH_KEYPOINTS)
            else:
                if FLAGS.gray_scale == 'yes':
                    features = cv2.drawKeypoints(gray, kp, blank_image, flags=cv2.DRAW_MATCHES_FLAGS_DEFAULT)
                else:
                    features = cv2.drawKeypoints(image, kp, blank_image, flags=cv2.DRAW_MATCHES_FLAGS_DEFAULT)
